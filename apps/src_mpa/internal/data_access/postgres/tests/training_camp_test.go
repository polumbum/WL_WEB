...
func (suite *TCampDATestSuite) SetupSuite() {
	...
	/*err = db.AutoMigrate(&entities.TCamp{})
	if err != nil {
		log.Fatal(err)
		return
	}

	err = db.AutoMigrate(&entities.TCampApplication{}, &entities.Sportsman{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *TCampDATestSuite) TearDownSuite() {
	/*err := suite.db.Migrator().DropTable(&entities.TCamp{})
	if err != nil {
		log.Fatal(err)
		return
	}
...
func (suite *TCampDATestSuite) TearDownTest() {
	err := suite.db.Where("city <> ?", "").Delete(&entities.TCamp{}).Error
	if err != nil {
		log.Fatal(err)
	}
}
...
func (suite *TCampDATestSuite) TestCreate() {
	...
	var fetchedTCamp entities.TCamp
...
func (suite *TCampDATestSuite) TestUpdate() {
	...
	var updatedTCamp entities.TCamp
...
func (suite *TCampDATestSuite) TestListTCamps() {
	tCamp1 := &entities.TCamp{City: "TCamp1"}
	tCamp2 := &entities.TCamp{City: "TCamp2"}
	err := suite.repo.Create(tCamp1)
	suite.NoError(err)
	err = suite.repo.Create(tCamp2)
	suite.NoError(err)

	tCamps, err := suite.repo.ListTCamps()
	suite.NoError(err)
	suite.NotNil(tCamps)
	suite.Len(tCamps, 2)
...
func (suite *TCampDATestSuite) TestRegisterSportsman() {
	...
	var createdTCampApp entities.TCampApplication
...
func (suite *TCampDATestSuite) TestDeleteRegistration() {
	...
	var notFoundTCampApp *entities.TCampApplication
