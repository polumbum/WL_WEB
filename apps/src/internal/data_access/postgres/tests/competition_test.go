...
func (suite *CompetitionDATestSuite) SetupSuite() {
	...
	/*err = db.AutoMigrate(&domain.Competition{})
	if err != nil {
		log.Fatal(err)
		return
	}

	err = db.AutoMigrate(&domain.Sportsman{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *CompetitionDATestSuite) TearDownSuite() {
	/*err := suite.db.Migrator().DropTable(&domain.Competition{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *CompetitionDATestSuite) TearDownTest() {
	...
	err = suite.db.Where("name <> ?", "").Delete(&domain.Competition{}).Error
	if err != nil {
		log.Fatal(err)
	}
}
...
func (suite *CompetitionDATestSuite) TestCreate() {
	...
	var fetchedCompetition domain.Competition
...
func (suite *CompetitionDATestSuite) TestUpdate() {
	...
	var updatedCompetition domain.Competition
...
func (suite *CompetitionDATestSuite) TestListCompetitions() {
	competition1 := &domain.Competition{Name: "Competition1"}
	competition2 := &domain.Competition{Name: "Competition2"}
	err := suite.repo.Create(competition1)
	suite.NoError(err)
	err = suite.repo.Create(competition2)
	suite.NoError(err)

	competitions, err := suite.repo.ListCompetitions(1, 10, "", "")
	suite.NoError(err)
...
func (suite *CompetitionDATestSuite) TestRegisterSportsman() {
	...
	var createdCompApp domain.CompApplication
...
func (suite *CompetitionDATestSuite) TestDeleteRegistration() {
	...
	var notFoundCompApp *domain.CompApplication
...
func (suite *CompetitionDATestSuite) TestGetUpcomingSuccess() {
	...
	competition1 := &domain.Competition{
		Name:              "Competition",
		City:              "Moscow",
		Address:           "Moscow Street",
...
func (suite *CompetitionDATestSuite) TestGetUpcomingSuccessFew() {
	...
	competition2 := &domain.Competition{
		Name:              "Competition",
		City:              "Moscow",
		Address:           "Moscow Street",
		BegDate:           time.Date(2000, time.November, 10, 0, 0, 0, 0, time.UTC),
