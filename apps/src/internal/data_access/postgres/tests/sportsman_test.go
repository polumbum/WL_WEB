...
func (suite *SportsmanDATestSuite) SetupSuite() {
	...
	err = db.AutoMigrate(&domain.Result{}, &domain.Competition{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *SportsmanDATestSuite) TearDownSuite() {
	/*err := suite.db.Migrator().DropTable(&domain.Sportsman{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *SportsmanDATestSuite) TearDownTest() {
	...
	err = suite.db.Where("name <> ?", "").Delete(&domain.Sportsman{}).Error
	if err != nil {
		log.Fatal(err)
	}
...
func (suite *SportsmanDATestSuite) TestCreate() {
	...
	var fetchedSportsman domain.Sportsman
...
func (suite *SportsmanDATestSuite) TestUpdate() {
	...
	_, err = suite.repo.Update(sportsman)
	suite.NoError(err)

	var updatedSportsman domain.Sportsman
...
func (suite *SportsmanDATestSuite) TestListSportsmen() {
	sportsman1 := &domain.Sportsman{Name: "Sportsman1"}
	sportsman2 := &domain.Sportsman{Name: "Sportsman2"}
	err := suite.repo.Create(sportsman1)
	suite.NoError(err)
	err = suite.repo.Create(sportsman2)
	suite.NoError(err)

	sportsmen, err := suite.repo.ListSportsmen(1, 10, "", "")
	suite.NoError(err)
