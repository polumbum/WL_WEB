...
func (suite *CoachDATestSuite) SetupSuite() {
	...
	/*err = db.AutoMigrate(&domain.Coach{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *CoachDATestSuite) TearDownSuite() {
	/*err := suite.db.Migrator().DropTable(&domain.Coach{})
	if err != nil {
		log.Fatal(err)
		return
	}*/
...
func (suite *CoachDATestSuite) TearDownTest() {
	...
	err = suite.db.Where("name <> ?", "").Delete(&domain.Coach{}).Error
	if err != nil {
		log.Fatal(err)
	}
}
...
func (suite *CoachDATestSuite) TestListCoaches() {
	coach1 := &domain.Coach{Name: "Coach1"}
	coach2 := &domain.Coach{Name: "Coach2"}
	err := suite.repo.Create(coach1)
	suite.NoError(err)
	err = suite.repo.Create(coach2)
	suite.NoError(err)

	coaches, err := suite.repo.ListCoaches(1, 10, "", "")
	suite.NoError(err)
